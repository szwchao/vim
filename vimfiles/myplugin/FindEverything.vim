...
if !exists('g:fe_openfile_filter')
   let g:fe_openfile_filter = ['txt', 'vim', 'c', 'h', 'py']
endif
...
fun! FindEverything()
   ...
   if matchstr(l:result, 'Everything IPC window not found, IPC unavailable.') != ""
      ...
      call s:Open_External(g:fe_et_exe)
      return
   endif
...
"FESyntax {{{2
fun! s:FESyntax()
   "exe ":syn keyword FE_Word ".g:fe_word
   "exe ":syn match FE_Word ". g:fe_word
   sy case ignore
   let s = ":syn match FE_Word ". '"'. g:fe_word .'"'
   exe s
   "highlight MyGroup ctermbg=green guibg=green
   hi def link FE_Word Type
   "let m = matchadd("FE_Word", g:fe_word)
   "syn match FE_Word ".*" contains=g:fe_word
endfun
"}}}
