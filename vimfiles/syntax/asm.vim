...
if version >= 508 || !exists("did_asm_syntax_inits")
  ...
  syn keyword asmKeywords	MOV PUSH POP XCHG XLAT
  syn keyword asmKeywords	CMP ADD ADC INC SUB SBB DEC NEG MUL DIV
  syn keyword asmKeywords	NOT AND OR XOR TEST SHL SAL SHR SAR ROL ROR RCL RCR
  syn keyword asmString		CALL RET
  syn keyword asmKeywords	JMP JA JAE JB JBE JG JGE JL JLE JC JNC JE JNE JZ JNZ 
  syn keyword asmKeywords	LOOP INT INTO
  syn keyword asmKeywords	CLC CTC CMC CLD STD CLI STI NOP WAIT
  syn keyword asmLabel          BYTE WORD PTR

  hi def link asmKeywords		Type
