...
function! s:insane_in_the_membrane() abort
  ...
  if get(g:, 'startify_show_files', 1) && !empty(v:oldfiles)
    ...
    for fname in v:oldfiles
      ...
      execute 'nnoremap <buffer>' cnt ':silent! edit' fnameescape(fname) . '<cr>'
      let cnt += 1
      if cnt == numfiles
        break
      endif
  ...
  if get(g:, 'startify_show_sessions', 1) && !empty(sfiles)
    ...
    for i in range(len(sfiles))
      ...
      execute 'nnoremap <buffer>' idx ':silent! source ' fnameescape(sfiles[i]) . '<cr>'
    endfor
    let cnt = idx
  endif
  ...
  "call cursor(6, 5)
  call cursor(4 + s:offset_header, 5)
endfunction
