...
function! s:close_tag_table(table, ldest) "{{{
  ...
  if len(table)
    ...
    if table[0] == 'center'
      call add(ldest, "<table class='table table-hover table-bordered table-striped center'>")
    else
      " table-bordered 表格带边框，table-striped 背景交替
      call add(ldest, '<table class="table table-hover table-bordered table-striped">')
    endif
...
function! s:process_tag_pre(line, pre) "{{{
  " pre is the list of [is_in_pre, indent_of_pre]  " pre的形式：[是否在pre里，大括号的缩进位置]
  let lines = []
  let pre = a:pre
  let processed = 0
  if !pre[0] && a:line =~ '^\s*{{{[^\(}}}\)]*\s*$'
    ...
    if class != ""
      ...
      "修改为默认加入google的pretty code样式
      call add(lines, '<pre class="prettyprint linenums">')
    endif
...
function! vimwiki#html#WikiAll2HTML(path, ...) "{{{
  ...
  " 是否总是更新html文件
  if a:0 == 1
    let always_update_wiki_to_html = 1
  else
    let always_update_wiki_to_html = 0
  endif
  ...
  for wikifile in wikifiles
    if !s:is_html_uptodate(wikifile) || always_update_wiki_to_html
